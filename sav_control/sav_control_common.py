...
def whoami():
    ...
    """
    result = run_cmd("ip -j -4 address", 30, capture_output=True)
...
def get_container_node_num(root_dir, capture_output=True):
    ...
    ret = run_cmd(cmd, capture_output=capture_output)
    if ret.returncode != 0:
        raise Exception(f"get_container_node_num failed, {ret.stderr}")
