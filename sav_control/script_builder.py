    ...
    for peer_id, peer_ip, dev_ip, dev_id, link_type in links:
        ...
        if is_inter:
            ...
            try:
                ...
                pass
        if link_type in RPDP_LINK_TYPES:
            has_rpdp = True
            if is_inter:
                has_inter_rpdp = True
        ...
        if has_inter_rpdp:
            if not dev_as in base["rep_router_map"]:
                base["rep_router_map"][dev_as] = dev_id
            if not peer_as in base["rep_router_map"]:
                base["rep_router_map"][peer_as] = peer_id
    sa_config["link_map"] = link_map
    sa_config["location"] = RT_CORE
    if has_inter_bgp:
        sa_config["location"] = RT_EDGE
    if has_inter_rpdp:
        if dev_id == base["rep_router_map"][dev_as]:
            sa_config["location"] = RT_EDGE_REP
    return bird_conf_str, sa_config,base
...
def write_compose_str(node, base, host_dir, run_dir, tag, compose_f, cpu_id, ca_ip):
    ...
    compose_str += f"    volumes:\n" \
        ...
        f"      - {host_dir}/sav_control/reference_and_agent/router_kill_and_start.sh:"\
...
def regenerate_config(
    ...
    for node_id in base["devices"]:
        ...
        sa_config = gen_sa_config(
            ...
            nodes)
        phy_links = [x for x in base["links"] if x[2] == LINK_PHYSICAL]
        bird_config_str, sa_config,base = add_links(base, node_id, bird_config_str,
