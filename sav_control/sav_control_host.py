...
class RunEmulation():
    ...
    def dev_test(self) -> None:
        ...
        self._ready_base(force_restart=True, build_image=True)
        self.send_start_signal()
        self._wait_valid_sav_table(skip_list=["r3","r4","r5"])
        t = 1
    ...
    def fib_stable(self, monitor_overlap_sec=10, if_monitor=False):
        ...
        self.send_stop_signal()
        run_cmd(f"docker compose -f {self.base_compose_path} down", None)
        if if_monitor:
            time.sleep(monitor_overlap_sec)
            self._stop_metric_monitor()
...
class DevicePerformance():
    ...
    def get_cpu_performance(self):
        ret = run_cmd(cmd=self.__cpucommand, timeout=None, capture_output=True)
        return ret.returncode, ret.stderr, ret.stdout

    def get_memory_performace(self):
        ret = run_cmd(cmd=self.__memorycommand, timeout=None, capture_output=True)
        return ret.returncode, ret.stderr, ret.stdout

    def get_disk_performance(self):
        ret = run_cmd(cmd=self.__diskcommand, timeout=None, capture_output=True)
        return ret.returncode, ret.stderr, ret.stdout

    def get_network_performance(self):
        ret = run_cmd(cmd=self.__networkcommand, timeout=None, capture_output=True)
        return ret.returncode, ret.stderr, ret.stdout

    def get_host_performance(self):
        ret = run_cmd(cmd=self.__hostCommand, timeout=None, capture_output=True)
        return ret.returncode, ret.stderr, ret.stdout

    def get_docker_container_performance(self):
        ret = run_cmd(cmd=self.__dockerStatscommand, timeout=None, capture_output=True)
        return ret.returncode, ret.stderr, ret.stdout
...
class Monitor:
    ...
    def protocol_step():
        ...
        for f in folders:
            ...
            cmd = f"grep LOG_FOR_FRONT {SAV_RUN_DIR}/{f_log_path}/sav-agent.log |awk -F\"LOG_FOR_FRONT\" '{{print $2}}'|grep -i SPA|grep -E \"'msg_cause': 'receive'|'msg_cause': 'relay'\""
            ret = run_cmd(cmd=cmd, capture_output=True)
            stdout = ret.stdout
            for i in stdout.replace("\'", "\"").split("\n"):
                if len(i) < 10:
                    continue
                step[f].append(json.loads(i))
            cmd = f"grep LOG_FOR_FRONT {SAV_RUN_DIR}/{f_log_path}/sav-agent.log |awk -F\"LOG_FOR_FRONT\" '{{print $2}}'|grep -i SPD| sed \"s/'/\\\"/g\"|grep -E '\"msg_cause\": \"origin\"|\"msg_cause\": \"relay\"'| jq -s 'unique_by(.src_ip, .dst_ip, .msg_cause, .link_name)'"
            ret = run_cmd(cmd=cmd, capture_output=True)
            stdout = ret.stdout
    ...
    def protocol_metric():
        ...
        ret = run_cmd(cmd=cmd, capture_output=True)
        for container_name in ret.stdout.split("\n")[:-1]:
            cmd = f"docker exec -i {container_name} curl http://localhost:8888/metric/"
            sub_ret = run_cmd(cmd=cmd, capture_output=True)
            metric.append({container_name: json.loads(stdout=sub_ret.stdout)})
    ...
    def protocol_table():
        ...
        for container_name in ret.stdout.split("\n")[:-1]:
            ...
            table.append(
                {
                    container_name: json.loads(
                        sub_ret.stdout.replace("IPNetwork(", "")
                        .replace(")", "")
                        .replace("'", '"')
                        .replace("True", '"True"')
                        .replace("False", '"False"')
    ...
    def enable_sav_table(protocol_name, router):
        ...
        for container_name in ret.stdout.split("\n")[:-1]:
            ...
            sub_ret = run_cmd(cmd=cmd, capture_output=True)
            result.append({container_name:json.loads(sub_ret.stdout)})
