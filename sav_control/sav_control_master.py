...
class MasterController:
    ...
    def _self_host(self, input_json):
        ...
        ret[SELF_HOST_ID] = script_builder(SAV_ROOT_DIR, SAV_OP_DIR, json_content,
    ...
    def _remote_run(self, node_id, node, cmd, capture_output=True):
        ret = {}
        if node_id == "localhost":
            ret["cmd_start_dt"] = time.time()
            run_ret = run_cmd(cmd, capture_output=capture_output)
            ret["cmd_result"] = {
                "returncode": run_ret.returncode,
                "stdout": run_ret.stdout,
                "stderr": run_ret.stderr,
                "cmd": cmd,
            }
    ...
    def mode_performance(self):
        for node_id in list(self.host_node.keys()):
            ...
            cmd = f'python3 {root_dir}/sav_control/sav_control_host.py -p all'
    ...
    def mode_start(self):
        ...
        for node_id, node_num in self.config["host_node"].items():
            ...
            path2hostpy = os.path.join(
                node["root_dir"], "sav_control", "sav_control_host.py")
...
class SavExperiment:
    ...
    def general_exp(self, base_cfg_name, skip_compile=True):
        ...
        for key, value in result.items():
            if value["cmd_result"]["returncode"] != 0:
                exit_code = -1
                break
        if exit_code == 0:
...
def run(args):
    ...
    cfg_path = os.path.join(SAV_OP_DIR, "sav_control_master_config.json")
    cfg = json_r(cfg_path)
    cfg["host_node"]["localhost"]["root_dir"] = SAV_ROOT_DIR
    json_w(cfg_path, cfg)
    ...
    if experiment is not None:
        ...
        match experiment:
            ...
            case _:
                config_file = None 
                if os.path.exists(os.path.join(SAV_OP_DIR, "base_configs", f"{experiment}.json")):
                    config_file = os.path.join(SAV_OP_DIR, "base_configs", f"{experiment}.json")
                elif os.path.exists(f"{experiment}.json"):
                    config_file = f"{experiment}.json"
                else:
                    raise FileNotFoundError(f"config file {config_file} not found")
                return sav_exp.general_exp(config_file, skip_compile=skip_compile)
