...
class SavAgent():
    ...
    def update_config(self):
        ...
        for i in range(3):
            try:
                ...
                if (not config["is_edge"]) and (config["location"] in [RT_EDGE_REP,RT_EDGE]):
                    raise ValueError(
                        f"not edge router but location is {config['location']}")
                ...
                self.router_id = netaddr.IPAddress(config["router_id"])
    ...
    def _refresh_aspa_info(self) -> None:
        ...
        """
        if not self.config["enable_rpki"]:
            self.logger.warning("rpki not enabled")
            return
    ...
    def find_prefix_info_in_adj_in(self, prefix)-> dict:
        ...
        self.logger.warning(f"prefix {prefix} not found in adj_in")
    def get_adj_in(self, protocol_name=None):
        ...
        if protocol_name not in self.data["adj_in"]:
            self.logger.debug(self.data["adj_in"].keys())
            self.logger.debug(protocol_name)
            raise KeyError()
        return copy.deepcopy(self.data["adj_in"][protocol_name])
    def _get_rid_by_link_name(self,link_name):
        remote_ip = str(self.link_man.get_by_name(link_name)['remote_ip'])
        # self.logger.debug(remote_ip)
        for r_info in self.config["as_scope"].values():
            if remote_ip in r_info["ips"]:
                return netaddr.IPAddress(r_info["router_id"])
        ...
        for l_meta in self.link_man.get_all_link_meta().values():
            ...
            if self.config["use_ignore_nets"]:
                for k in list(ret.keys()):
                    if k in [netaddr.IPNetwork("0.0.0.0/0"), netaddr.IPNetwork("::/0")]:
                        del ret[k]
                        continue
